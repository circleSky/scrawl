...
public class CopyLocation {
    ...
    public void drawItSelf(Canvas canvas, float size) {
        ...
        DrawUtil.drawCircle(canvas, mX, mY, size / 2 + size / 8, mPaint);

        mPaint.setStrokeWidth(size / 16);
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setColor(0xaaffffff); // 白色
        DrawUtil.drawCircle(canvas, mX, mY, size / 2 + size / 32, mPaint);

        mPaint.setStyle(Paint.Style.FILL);
        if (!mIsCopying) {
            mPaint.setColor(0x44ff0000); // 红色
            DrawUtil.drawCircle(canvas, mX, mY, size / 2, mPaint);
        } else {
